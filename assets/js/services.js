(function () {
  ...
  function renderServicePage(data, slug) {
    ...
    main.innerHTML = [
      ...
      '<section class="section section-soft"><div class="container"><div class="section-heading"><div><p class="eyebrow">What to compare</p><h2>Decision factors homeowners often review.</h2></div><p>Provider responses can vary by location, access, project complexity, and scope. Oakline keeps those details organized before you decide whether to continue.</p></div><div class="compare-grid">' + iconList(data.compare) + '</div></div></section>',
      ...
      '<section class="section section-soft"><div class="container matching-panel" data-aos="fade-up"><div><p class="eyebrow">Matching process</p><h2>How Oakline routes this type of request.</h2><p>Submitting a request does not create a direct service contract with Oakline. Any follow-up, quote, scheduling, and terms come from participating providers.</p></div><div class="process-line">' + data.steps.map(function (step, index) { return '<article class="process-step"><span>0' + (index + 1) + '</span><h3>' + step.split(".")[0] + '</h3><p>' + step + '</p></article>'; }).join("") + '</div></div></section>',
